 ...
 */
function hasZoneContent(entry: Partial<IEntrySerializable>, zoneId: string): boolean {
 ...
 */
export function detectTemplateFromEntry(entry: Partial<IEntrySerializable>): string {
    ...
    // Count active zones
    const activeZones = [hasPictureText, hasList, hasTextRight, hasFreeForm].filter(Boolean).length;
...
export function getActiveZones(entry: Partial<IEntrySerializable>): string[] {
    ...
    if (hasZoneContent(entry, 'free_form_content')) zones.push('free_form_content');
...
export function wouldLoseDataOnTemplateSwitch(
    ...
    for (const zone of activeZones) {
        ...
        const zoneKey = zone === 'free_form_content' ? 'free_form_content' : zone;
        if (!newTemplate.zones[zoneKey as keyof typeof newTemplate.zones]?.enabled) {
            return true;
        }
    }
