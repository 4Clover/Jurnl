...
export async function seedTestUsers(currentUserId?: string): Promise<void> {
    ...
    try {
        ...
        if (existingUsers.length > 0) {
            console.log('✅ Test users already exist:', existingUsers.map(u => u.username));
        ...
        for (const testUser of testUsers) {
            ...
            const journal = new Journal({
                ...
                cover_color: ['#3B82F6', '#10B981', '#F59E0B', '#EF4444'][Math.floor(Math.random() * 4)],
                user: savedUser._id
            });
            ...
            const sampleEntries = [
                {
                    ...
                    shared_with_friends: currentUserId ? [currentUserId] : []
                },
                {
                    ...
                    content_zones: {
                        ...
                        list: { 
                            items: [
                                { text: 'Had coffee this morning', checked: true },
                                { text: 'Read an interesting article', checked: true },
                                { text: 'Plan weekend activities', checked: false }
                            ]
                        },
                        text_right: { content: 'Looking forward to sharing more thoughts!' }
                    },
                    free_form_content: `Reflecting on today's events and planning for tomorrow. Hope my friends enjoy reading this!`,
                    user: savedUser._id,
                    journal: savedJournal._id,
                    entry_date: new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString(),
                    shared_with_friends: currentUserId ? [currentUserId] : []
                },
                {
            ...
            console.log(`✅ Created user: ${testUser.username} with journal and sample entries`);
        }
        ...
        console.log('📝 You can now test friends functionality with these users:');
        testUsers.forEach(user => {
            console.log(`   - ${user.username} (${user.username_display})`);
        });
        if (currentUserId) {
            console.log('👥 All test users have been added as friends to current user');
...
async function addFriendsToCurrentUser(currentUserId: string, testUsers: (Document & IUser)[]): Promise<void> {
    try {
...
export async function clearTestData(): Promise<void> {
    try {
        ...
        const usersToDelete = await User.find({ username: { $in: testUsernames } });
        ...
        // Delete entries
        const deletedEntries = await Entry.deleteMany({ user: { $in: userIds } });
        console.log(`🗑️ Deleted ${deletedEntries.deletedCount} entries`);
        
        // Delete journals
        const deletedJournals = await Journal.deleteMany({ user: { $in: userIds } });
        console.log(`🗑️ Deleted ${deletedJournals.deletedCount} journals`);
        
        // Delete users
        const deletedUsers = await User.deleteMany({ username: { $in: testUsernames } });
