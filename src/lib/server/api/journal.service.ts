...
export class JournalServiceWithEntries {
    ...
    async listWithRecentEntries(event: RequestEvent, filter: FilterQuery<IJournal> = {}) {
        try {
            ...
            for (const journal of journals) {
                ...
                if (!this.options.canRead || this.options.canRead(journalDoc, event)) {
                    const serialized: SerializedJournal = {
                        _id: journalId,
                        title: String(journal.title),
                        cover_color: String(journal.cover_color),
                        description: journal.description ? String(journal.description) : undefined,
                        user: String(journal.user),
                        createdAt: journal.createdAt,
                        updatedAt: journal.updatedAt,
                        entries: Array.isArray(journal.entries) ? 
                            (journal.entries as unknown as PopulatedEntry[]).map((entry): SerializedEntry => ({
