...
export const load: PageServerLoad = async ({ params, locals }) => {
    ...
    try {
        ...
        const friend = await User.findOne({ username: friendUsername })
            .select('username username_display bio_text bio_image_url avatar_url');
        ...
        const currentUser = await User.findById(locals.user.id).select('close_friends');
        ...
        const sharedEntries = await Entry.find({
            user: friend._id,
            shared_with_friends: locals.user.id
        })
        ...
        const friendInfo = {
            ...
            bio_image_url: friend.bio_image_url || friend.avatar_url || "https://i.pinimg.com/736x/6c/21/68/6c21684b57384c2d91d6d86ef2cbe2a4.jpg"
        };
